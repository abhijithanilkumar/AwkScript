...
END{
	...
	for(node=0;node<total_nodes;++node)
	{
        for(tstep=0;tstep<=(19/interval);++tstep)
        {
	...
	system("gnuplot -persist -e \"n="total_nodes"\" sent.plt")
	system("gnuplot -persist -e \"n="total_nodes"\" recv.plt")
}
